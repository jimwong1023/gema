...
slate.Variants = (function() {
  ...
  function Variants(options) {
    this.METALS = ["14K Yellow Gold", "14K White Gold", "14K Rose Gold", "Platinum"];
    this.$container = options.$container;
    this.product = options.product;
    this.singleOptionSelector = options.singleOptionSelector;
    this.swatchSpan = options.swatchSpan;
    this.originalSelectorId = options.originalSelectorId;
    this.enableHistoryState = options.enableHistoryState;
    this.currentVariant = this._getVariantFromOptions();

    this._updateSwatchSpan(this.currentVariant);
  ...
  Variants.prototype = _.assignIn({}, Variants.prototype, {
    ...
    _getCurrentOptions: function() {
      var currentOptions = _.map($(this.singleOptionSelector, this.$container), function(element) {
        ...
        } else if (type === undefined && $element.hasClass('metal-swatch-radio-inputs')) {
          var $selectedEl = $element.find('input[name=metal-swatch]:checked');

          currentOption.value = $selectedEl.val();
          currentOption.index = $selectedEl.data('index');
    ...
    _onSelectChange: function() {
      ...
      this._updateCompareablePrice(variant);
      this._updateSwatchSpan(variant);
    ...
    _updateSwatchSpan: function(variant) {
      var $swatchSpan = $(this.swatchSpan);
      
      if ($swatchSpan.length > 0) {
        var foundMetal = false;

        for (i=0; i<this.METALS.length; i++) {
          var metal = this.METALS[i];

          if (variant.options.includes(metal)) {
            foundMetal = metal;
          }
        }

        if (foundMetal !== false) {
          $swatchSpan.text(':  ' + foundMetal);
        }
      }
    },
     ...
     * @return {event}  variantCompareablePriceChange
     */
    _updateCompareablePrice: function(variant) {
      if (window.variantCompareablePrices) {
        var compareablePrice = window.variantCompareablePrices[variant.id];
        var currentCompareablePrice = document.getElementById('variant-compareable-price').innerHTML;

        this.$container.trigger({
          type: 'variantCompareablePriceChange',
          variant: variant,
          comepareablePrice: compareablePrice
        });
      } else {
        document.getElementById('header-compareable-price').classList.add('visibility-hidden')
      }
    },
    ...
    _updateImages: function(variant) {
      ...
      this.$container.trigger({
        type: 'variantSelectedImageChange',
        variant: variant
      });
    },
...
theme.Header = (function() {
  var selectors = {
    ...
    siteNavDropdowns: '.site-nav__dropdown'
  };
...
theme.MobileNav = (function() {
  ...
  function openMobileNav() {
    var translateHeaderHeight = ($('.announcement-bar').height() + $('.c-site-header-padding').height() - 1) || 0;
    ...
    $("." + classes.bodyOverlay).on('click', toggleMobileNav);
...
theme.Product = (function() {
  function Product(container) {
    ...
    this.selectors = {
      ...
      swatchSpan: '#swatchSpan-' + sectionId,
      SKU: '.variant-sku',
  ...
  Product.prototype = _.assignIn({}, Product.prototype, {
    ...
    _initUnhideImages: function() {
      $('#js-hidden-product-images').removeClass('visibility-hidden');
    },
    ...
    _initVariants: function() {
      var options = {
        ...
        swatchSpan: this.selectors.swatchSpan,
      ...
      this.$container.on('variantSelectedImageChange' + this.settings.namespace, this._updateSelectedImages.bind(this));
      this.$container.on('variantPriceChange' + this.settings.namespace, this._updatePrice.bind(this));
      this.$container.on('variantSKUChange' + this.settings.namespace, this._updateSKU.bind(this));
      this.$container.on('variantCompareablePriceChange' + this.settings.namespace, this._updateCompareablePrice.bind(this));
    },
    ...
    _initImageSwitch: function() {
      ...
      var self = this;
      this._updateSelectedImages({ variant: this.variants.currentVariant });
    ...
    _updateCompareablePrice: function(evt) {
      var compareablePrice = evt.comepareablePrice;
      if (compareablePrice === "") {
        document.getElementById('header-compareable-price').classList.add('visibility-hidden');
      } else {
        $('#variant-compareable-price').html(compareablePrice);
        document.getElementById('header-compareable-price').classList.remove('visibility-hidden');
      }
    },
    ...
    _updateSelectedImages: function(evt) {
      var variant = evt.variant;
      var $newImage, $otherImages;

      
      $newImage = this._findProductImageOrThumbnailWrap(variant, $(this.selectors.productImageWraps));
      
      if ($newImage) {
        $otherImages = $(this.selectors.productImageWraps + ':not([data-image-id=\'' + $newImage.data('imageId') + '\'])');

        this._updateSelectedThumbnails(evt);
        $newImage.removeClass('hide');
        $otherImages.addClass('hide');
      }
    },
    
    _updateSelectedThumbnails: function(evt) {
      var variant = evt.variant;
      var newThumb;

      newThumbs = this._findAllProductImagesOrThumbnailWraps(variant, $(this.selectors.productThumbImages));

      if (newThumbs) {      
        $(this.selectors.productThumbImages).each(function(i, productThumbImage) {
          var $productThumbImage = $(productThumbImage);
          $productThumbImage.addClass('hide');
          $productThumbImage.parent().addClass('hide');
        });

        $.each(newThumbs, function(i, $thumb) {
    ...
    _findProductImageOrThumbnailWrap: function(variant, $wraps) {
      for(i=0; i<$wraps.length; i++) {
        var $wrap = $($wraps[i]);
        var imageAlt = $wrap.data('imageAlt').toLowerCase();

        for(j=0; j<variant.options.length; j++) {
          var option = variant.options[j].toLowerCase();
          if(imageAlt.includes(option)) {
            return $wrap
          }
        }
      }
    ...
    _findAllProductImagesOrThumbnailWraps: function(variant, $wraps) {
      var wraps = [];
      for(i=0; i<$wraps.length; i++) {
        var $wrap = $($wraps[i]);
        var imageAlt = $wrap.data('imageAlt').toLowerCase();
